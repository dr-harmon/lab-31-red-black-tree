...
private:
    ...
    public:
        ...
        void dumpNode(std::ostream& graphFile);
        #endif
...
template <typename T>
void RBTree<T>::RBTreeNode::toString(std::ostream& buffer, const std::string& prefix, bool lastNode) {
    //print the current element and the children
    buffer << prefix << (lastNode ? "└── " : "├── ") << key << (color == RED ? " (R)" : " (B)") << std::endl;
...
template <typename T>
void RBTree<T>::RBTreeNode::dumpNode(std::ostream& graphFile) {
    ...
    if (left != NULL) {
        graphFile << key << " -> " << left->key << std::endl;
        left->dumpNode(graphFile);
    }

    if (right != NULL) {
        graphFile << key << " -> " << right->key << std::endl;
...
void RBTree<T>::dumpTree() {
    ...
    graphFile << "node [style=filled, penwidth=2, fontcolor=white, fontname=\"Arial Black\"];" << std::endl;
    graphFile << "graph [pad=\"0.1\", nodesep=\"1\", ranksep=\"1.5\"];" << std::endl;
    ...
    std::cout << graphFile.str() << std::endl;
}
